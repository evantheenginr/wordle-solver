...
module.exports = class WordleBoard {
    ...
    calcLetterScores(){
        ...
        switch(this.config.LetterScoreAlgorithm){
            ...
            case "DYNAMIC_PROPORTINAL_WORDLE_LIST":
                this.scores = new Map(alphabet.map(letter => [letter, Math.round(this.words.filter(word => word.includes(letter)).length / this.words.length ) ] ))
                break
            default:
    ...
    /**
     * Get the next word(s) to guess.
     * @returns {Array<string>} The words that match the board constraints
     */
    solve(){
        this.log.debug(`returning word solutions, words ramining: ${this.words.length}`)
        return this.words
    }
}